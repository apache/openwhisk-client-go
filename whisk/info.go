...
func (s *InfoService) Get() (*Info, *http.Response, error) {
    ...
    if err != nil {
        Debug(DbgError, "url.Parse(%s) error: %s\n", s.client.Config.Version, err)
         fmt.Println("Unable to URL parse '{{.version}}': {{.err}}")
        werr := MakeWskError(errors.New("Unable to URL parse '{{.version}}': {{.err}}"), EXITCODE_ERR_GENERAL, DISPLAY_MSG, NO_DISPLAY_USAGE)
        return nil, nil, werr
    }
